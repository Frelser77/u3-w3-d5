...
function MusicSection({ genre, isSearchResult }) {
	...
	useEffect(() => {
		if (isSearchResult) {
			...
			// Altrimenti, carica la sezione musicale statica specificata
	...
	return (
		<Row className="justify-content-center align-items-center">
			<Col xs={10}>
				<div id={genre}>
					<h2 className="text-light">{genre === "HipHop" ? "#" + genre : genre}</h2>
					<div className="row row-cols-1 row-cols-sm-2 row-cols-lg-3 row-cols-xl-4 imgLinks py-3">
						{musicData &&
							sliceAlbumsToShow(musicData, 4).map((songInfo) => <AlbumCard key={songInfo.id} songInfo={songInfo} />)}
					</div>
				</div>
			</Col>
		</Row>
